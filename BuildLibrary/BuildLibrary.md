...
4.
Create title(), isCheckedOut(), and ratings() getter methods. Each getter should return the value saved to the corresponding property from the previous step.

5.
Create an isCheckedOut() setter that updates the _isCheckedOut property.

6.
Under your getters, create a method called toggleCheckOutStatus() that changes the value saved to the _isCheckedOut property.

If the current value is true, then change it to false. If the current value is false, then change it to true.
...
8.
Let’s add a method named addRating() that accepts one argument, then uses .push() and the ratings getter to add it to the end of the _ratings array.
