...
count = 0;
storyWords.forEach((token) => {
  return count++
});

storyWords = storyWords.filter((token) => {
  return token !== unnecessaryWord; 
})

storyWords = storyWords.map((token) => {
  return token === misspelledWord ? 'beautiful' : token;
})

let badWordIndex = storyWords.findIndex((token) => {
  return token === badWord;
});
console.log(badWordIndex);
storyWords[78] = 'realy';

let lengthCheck = storyWords.every((token) => {
  return token.length <= 10;
})
console.log(lengthCheck);

storyWords.forEach((token) => {
  token.length > 10 && console.log(token);
})
